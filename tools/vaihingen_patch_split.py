...
def get_img_mask_padded(image, mask, patch_size, mode):
    ...
    pad_img = albu.PadIfNeeded(min_height=h, min_width=w, position='bottom_right', border_mode=cv2.BORDER_CONSTANT, fill=0)(image=img)
    pad_mask = albu.PadIfNeeded(min_height=h, min_width=w, position='bottom_right', border_mode=cv2.BORDER_CONSTANT, fill=6)(image=mask)
...
def randomsizedcrop(image, mask):
    ...
    crop = albu.RandomSizedCrop(min_max_height=(int(3*h//8), int(h//2)), size=(h,w))(image=image.copy(), mask=mask.copy())
