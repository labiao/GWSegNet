...
def train_aug(img, mask):
    crop_aug = Compose([RandomScale(scale_list=[0.5, 0.75, 1.0, 1.25, 1.5], mode='value'),
                        SmartCropV1(crop_size=504, max_ratio=0.75,
...
def val_aug(img, mask):
    crop_aug = Compose([Resize((504, 504))])
    img, mask = crop_aug(img, mask)
    if mask is None:
        img = np.array(img)
...
class VaihingenDataset(Dataset):
    def __init__(self, data_root='data/vaihingen/test', mode='val', img_dir='images', mask_dir='masks',
        ...
        if 'gansu' in self.data_root:
            self.img_suffix = '.tif'
            self.mask_suffix = '.tif'
    ...
    def load_img_and_mask(self, index):
        img_id = self.img_ids[index]
        img_name = osp.join(self.data_root, self.img_dir, img_id.replace('mask_', '') + self.mask_suffix)
