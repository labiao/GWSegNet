...
class GWSegNetLoss(nn.Module):
    ...
    def forward(self, logits, labels):
        if self.training and len(logits) == 2:
            _, H, W = labels.shape
            logit_main, logit_aux = logits
            labels_32x = Resize((H // 28, W // 28), interpolation=InterpolationMode.NEAREST, antialias=True)(labels)
            # print(labels.shape, labels_32x.shape)  # torch.Size([8, 504, 504]) torch.Size([8, 15, 15])
...
# Backward compatibility for the retained UrbanSSF benchmark configs.
UMambaLoss = GWSegNetLoss
