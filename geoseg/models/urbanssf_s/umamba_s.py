...
class MF(nn.Module):
    ...
    def forward(self, x, res):
        ...
        res = self.pre_conv(res)  # (8，226，64，64) --> (8，128，64，64)
        x = self.mamba(x, self.attn(res))  # self.attn(res)维度是(8，128，64，64)  最终得到的x与输入x维度相同
...
class FeatureDetailDistiller(nn.Module):
    ...
    def forward(self, x, res):
        ...
        x = torch.cat([x, self.attn(res)], dim=1)  # 得到 (8,256,256,256)
...
class Decoder(nn.Module):
    ...
    def forward(self, res1, res2, res3, res4, h, w):
        ...
        x = self.attn(x.flatten(2).permute(0, 2, 1)).permute(0, 2, 1).view(B, C, H, W)  # (8,128,32,32) 维度没变
        if self.training:
            # 这个地方会增加参数量，res4通道更多
            ah = self.h(x)  # 辅助分类器
