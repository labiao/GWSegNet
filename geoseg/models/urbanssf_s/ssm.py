...
class Block(nn.Module):
    ...
    def forward(
        ...
        """
        if not self.fused_add_norm: #进来了
            if residual is None:
                residual = hidden_states
            else:
                residual = residual + self.drop_path(hidden_states)  # hidden_states （8,4096,128）
        ...
        hidden_states = self.mixer(hidden_states, inference_params=inference_params)  # mamba 推理
...
class Mamba_Block(nn.Module):
    ...
    def forward(self, x, residual=None):
        ...
        hidden_states = x.flatten(2).transpose(1, 2)  # （8,4096,128）
